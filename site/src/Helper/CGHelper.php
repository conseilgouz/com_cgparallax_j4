...
class CGHelper
{
    ...
    public static function getParallax($params)
        ...
        foreach ($sectionsList as $item) {
            ...
            if ($item->cg_img_type == "one") {
                ...
                foreach($item->slideslist as $uneimage) {
                    if (strlen(trim($uneimage->file_name)) == 0) {
                        continue;
                    }
            ...
            if ($item->cg_img_type != "none") {
                $parallax_sections[$ix] = array("slides" => $images, "delay" => $delay, "transitionDuration" => $duration,"cover" => true, "transition" => $trans,"animation" => $animation,"loop" => true,"lighten" => $item->lighten);
            }
            $result .= "</div>";
            $ix += 1;
        }
